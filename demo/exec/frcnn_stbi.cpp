...
int main(int argc, const char *argv[]) {
    ...
    if (width < height)
    {
        scale = (float)input_H / width;
    }
    else
    {
        scale = (float)input_H / height;
    }

    Matrix trans;
    // Set scale, from dst scale to src
    trans.setScale((float)(width-1) / (input_W-1), (float)(height-1) / (input_H-1));
    ImageProcess::Config pconfig;
    pconfig.filterType = CV::BILINEAR;
    //        float mean[3]     = {103.94f, 116.78f, 123.68f};
    //        float normals[3] = {0.017f, 0.017f, 0.017f};
    float mean[3] = {102.9801f, 115.9465f, 122.7717f};
    //float normals[3] = {0.00785f, 0.00785f, 0.00785f};
    float normals[3] = {1.0f, 1.0f, 1.0f};
    ::memcpy(pconfig.mean, mean, sizeof(mean));
    ::memcpy(pconfig.normal, normals, sizeof(normals));
    pconfig.sourceFormat = RGBA;
    pconfig.destFormat   = RGB;
    
    std::shared_ptr<ImageProcess> pretreat(ImageProcess::create(pconfig));
    ...
    input1->host<float>()[0] = input_H;
    input1->host<float>()[1] = input_W;
    ...
    //Image PostProcess
    {
        ...
        auto rois_host = std::make_shared<MNN::Tensor>(rois, rois->getDimensionType());
        ...
        auto pred_host = std::make_shared<MNN::Tensor>(bbox_preds, bbox_preds->getDimensionType());
        ...
        auto cls_host = std::make_shared<MNN::Tensor>(cls_probs, cls_probs->getDimensionType());
